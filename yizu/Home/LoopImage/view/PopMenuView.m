                       ...
                       action:(void(^)(NSInteger index))action
{
    ...
    if (self = [super init]) {
        ...
        // 创建tableView
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(kSCREEN_WIDTH - width - 5, point.y + 10, width, kCellHeight * array.count) style:UITableViewStylePlain];
...
+ (void)showWithItems:(NSArray <NSDictionary *>*)array
     ...
     triangleLocation:(CGPoint)point
               action:(void(^)(NSInteger index))action
{
    PopMenuView *view = [[PopMenuView alloc] initWithItems:array width:width triangleLocation:point action:action];
    [view show];
}
...
- (void)show {
    ...
    _tableView.layer.position = CGPointMake(kSCREEN_WIDTH - 5, _trianglePoint.y + 10);
    // 向右下transform
