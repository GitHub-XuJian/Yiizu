...
- (void)initialization {
    ...
    _chartHeight = self.bounds.size.height - BAR_CHART_TOP_PADDING - BAR_CHART_TEXT_HEIGHT-20;
    ...
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(BAR_CHART_LEFT_PADDING, 0, width - BAR_CHART_RIGHT_PADDING - BAR_CHART_LEFT_PADDING, CGRectGetHeight(self.bounds)-20)];
...
- (void)reloadChartYAxis {
    ...
    label.frame = CGRectMake(10, 0, kSCREEN_WIDTH/2, 20);
    ...
    for (int i = 0; i < _titleArray.count; i++) {
        if ([self.delegate respondsToSelector:@selector(barChartView:colorOfBarInSection:index:)]) {
            ...
            view.frame = CGRectMake(BAR_CHART_LEFT_PADDING - 2+(kSCREEN_WIDTH-20-BAR_CHART_LEFT_PADDING - 2)/_titleArray.count*i, _scrollView.y+_scrollView.height, 10, 10);
            view.backgroundColor = [self.delegate barChartView:self colorOfBarInSection:0 index:i];
            //设置圆角
            ...
            CGFloat textLabel_W = [EncapsulationMethod calculateRowWidth:str andFont:13 andHight:20];
            UILabel *textLabel = [[UILabel alloc] init];
            textLabel.frame = CGRectMake(view.x+view.width+5, view.y-5, textLabel_W, 20);
            textLabel.backgroundColor = [UIColor clearColor];
            textLabel.font = kFontOther;
            textLabel.text = str;
    ...
    for (NSInteger i = 0; i <= _numberOfYAxis; i ++) {
        ...
        UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(textLabel.x+textLabel.width, chartYOffset - unitHeight * i, kSCREEN_WIDTH-textLabel.x-textLabel.width-20, 0.5)];
        lineLabel.backgroundColor = kColorLine;
