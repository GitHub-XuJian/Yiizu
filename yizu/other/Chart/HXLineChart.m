...
- (void)setTitleArray:(NSArray *)titleArray{
    ...
    for (int i = 0; i < titleArray.count; i++) {
        [linePath moveToPoint:CGPointMake(_x + _margin + (_lineWidth - _margin * 2) / (titleArray.count - 1) * i, Figure_Down)];
        [linePath addLineToPoint:CGPointMake(_x + _margin + (_lineWidth - _margin * 2) / (titleArray.count - 1) * i, _lineHeight+Figure_Down)];
    }
...
- (void)setValue:(NSArray *)valueArray withYLineCount:(int)count{
    ...
    for (int i = 1; i < count - 1; i++) {
        [linePath moveToPoint:CGPointMake(_x , _lineHeight / (count - 1) * i+Figure_Down)];
        [linePath addLineToPoint:CGPointMake(_x + _lineWidth,_lineHeight / (count - 1) * i+Figure_Down)];
    }
    ...
    for (int i = 0; i < count; i++) {
        UILabel *valueLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, i * (_lineHeight / (count - 1)) - valueHeight / 2+Figure_Down, valueWidth, valueHeight)];
    ...
    [lPath addLineToPoint:CGPointMake(_x + _margin, _lineHeight - _lineHeight * ([valueArray[0] floatValue] / _maxValue)+Figure_Down)];
    [path addLineToPoint:CGPointMake(_x + _margin, _lineHeight - _lineHeight * ([valueArray[0] floatValue] / _maxValue)+Figure_Down)];
    
    for (int i = 1; i < valueArray.count ; i++) {
        [lPath addLineToPoint:CGPointMake(_x + _margin + (_lineWidth - _margin * 2) / (_titleCount - 1) * i,_lineHeight - _lineHeight * [valueArray[i] floatValue] / _maxValue+Figure_Down)];
        
        [path addLineToPoint:CGPointMake(_x + _margin + (_lineWidth - _margin * 2) / (_titleCount - 1) * i,_lineHeight - _lineHeight * [valueArray[i] floatValue] / _maxValue+Figure_Down)];
        
        if (i == valueArray.count - 1) {
            [lPath addLineToPoint:CGPointMake(_width,_lineHeight - _lineHeight * [valueArray[i] floatValue] / _maxValue+Figure_Down)];
            [path addLineToPoint:CGPointMake(_width,_lineHeight - _lineHeight * [valueArray[i] floatValue] / _maxValue+Figure_Down)];
            [path addLineToPoint:CGPointMake(_width,_lineHeight+Figure_Down)];
            [path addLineToPoint:CGPointMake(_x,_lineHeight+Figure_Down)];
            [path moveToPoint:CGPointMake(_x, _lineHeight - _lineHeight * ([valueArray[0] floatValue] / _maxValue)+Figure_Down)];
        }
    }
...
-(void)drawLine
{
    ...
    _lineHeight = _height - 60;
    
    //参照线
    UIBezierPath *linePath = [UIBezierPath bezierPath];
    [linePath moveToPoint:CGPointMake(_x,Figure_Down)];
    [linePath addLineToPoint:CGPointMake(_x + _lineWidth,Figure_Down)];
    [linePath moveToPoint:CGPointMake(_x + _lineWidth,_lineHeight+Figure_Down)];
    [linePath addLineToPoint:CGPointMake(_x,_lineHeight+Figure_Down)];
